...
function UpdateSessionModal(props) {
  ...
  const [newDescriptionEn, setnewDescriptionEn] = useState(
    props.sessions.description_en
  );
  const [newDescriptionAr, setnewDescriptionAr] = useState(
    props.sessions.description_ar
  );
  ...
  useEffect(() => {
    Axios.get("http://localhost:8000/api/psychologist").then((response) => {
  ...
  const update = async (e) => {
    ...
    try {
      await Axios.post(
        `http://localhost:8000/api/support-session/${props.id}?_method=PUT`,
        data
      ).then((response) => {
        if (response.data.status === "Token is Expired") {
          expireToken();
