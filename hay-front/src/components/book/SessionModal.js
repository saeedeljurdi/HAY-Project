...
function SessionModal(props) {
    ...
    return (
        <div className="sessionmodal" >
            <div style={{ display:"grid" ,gridTemplateColumns:"1fr 1fr 1fr" ,paddingBottom:"5%" }}>
                {data.map((val) => {
                    ...
                    return <div className="session-div" key={val.id}>
                        <div style={{marginLeft:"5%" , height:"fit-content"}} className="inner-session">{language === true ? <span>{val.name_en}</span> : <span>{val.name_ar}</span>}
                            {val.image === "empty" ? (
                                <img style={{ width: "100%", margin: "0 auto" }} src={Session} alt="error_profile" />
                            ) : (
                            ...
                            {language === true ? <span>{descEn}</span> : <span>{descAr}</span>}</div>
                            
                        <SessionTransport lang={props.language} psy={val.psychologist} data={val} />
                    </div>
